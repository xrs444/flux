...
# skip Helm chart templates (Go template syntax) and CI workflow files
...
# skip patch files (strategic merge patches are not complete resources)
find ./cluster -maxdepth 2 -type f -name '*.yaml' \
  -not -name '*-patch.yaml' \
  -print0 | while IFS= read -r -d $'\0' file;
  do
