...
APPS = [
    {"name": "paperless",      "client_id": "oauth2_paperless",      "redirect_uri": "https://paperless.xrs444.net/accounts/oidc/kanidm/login/callback/",         "scopes": "openid profile email"},
    {"name": "mealie",         "client_id": "oauth2_mealie",         "redirect_uri": "https://mealie.xrs444.net/login",                                            "scopes": "openid profile email"},
    {"name": "netbox",         "client_id": "oauth2_netbox",         "redirect_uri": "https://netbox.xrs444.net/oauth/complete/oidc/",                             "scopes": "openid profile email"},
    {"name": "immich",         "client_id": "oauth2_immich",         "redirect_uri": "https://immich.xrs444.net/auth/login",                                       "scopes": "openid profile email"},
    {"name": "romm",           "client_id": "oauth2_romm",           "redirect_uri": "https://romm.xrs444.net/oauth/callback",                                     "scopes": "openid profile email"},
    {"name": "audiobookshelf", "client_id": "oauth2_audiobookshelf", "redirect_uri": "https://audiobookshelf.xrs444.net/audiobookshelf/auth/openid/callback",      "scopes": "openid profile email"},
    {"name": "booklore",       "client_id": "oauth2_booklore",       "redirect_uri": "https://booklore.xrs444.net/oauth2-callback",                                "scopes": "openid profile email"},
    {"name": "matrix",         "client_id": "oauth2_matrix",         "redirect_uri": "https://matrix.xrs444.net/_synapse/client/oidc/callback",                   "scopes": "openid profile email"},
...
def _totp_now(secret_b32: str, interval: int = 30) -> int:
    ...
    offset = mac[-1] & 0x0F
    return (struct.unpack(">I", mac[offset:offset + 4])[0] & 0x7FFF_FFFF) % 1_000_000
...
def check_authorize(session: requests.Session, token: str, app: dict) -> tuple[bool, str]:
    ...
    params = {
        ...
        "redirect_uri": app["redirect_uri"], "scope": app["scopes"],
        "state": state, "code_challenge": challenge, "code_challenge_method": "S256",
    }
    try:
        resp = session.get(f"{KANIDM_URL}/oauth2/authorise", params=params,
    ...
    if resp.status_code == 302:
        if "error=" in location:
            ...
            return False, f"oauth2 error={qs.get('error',['?'])[0]} -- {qs.get('error_description',[''])[0] or location[:100]}"
...
def push_metrics(metrics: list[dict]) -> None:
    lines = (
        ["# HELP synthetic_login_success 1=ok 0=fail", "# TYPE synthetic_login_success gauge"]
        + [f'synthetic_login_success{{app="{m["app"]}"}} {m["success"]}' for m in metrics]
        + ["# HELP synthetic_login_duration_seconds authorize request wall-clock time", "# TYPE synthetic_login_duration_seconds gauge"]
        + [f'synthetic_login_duration_seconds{{app="{m["app"]}"}} {m["duration"]:.4f}' for m in metrics]
    )
    requests.put(f"{PUSHGATEWAY_URL}/metrics/job/synthetic_login",
                 data="\n".join(lines) + "\n",
